...
export const studentService = {
  ...
  search: (filters: SearchStudentDto): Student[] => {
    return students.filter((s) => {
      if (
        filters.firstName &&
        !s.firstName.toLowerCase().includes(filters.firstName.toLowerCase())
      )
        return false;
      if (
        filters.lastName &&
      ...
      if (
        ...
        return false;
      if (filters.grade !== undefined && s.grade !== filters.grade)
        return false;
  ...
  getStats: (): Stats => {
    ...
    const averageGrade =
      Math.round(
        (students.reduce((sum, s) => sum + s.grade, 0) / total) * 100,
      ) / 100;
